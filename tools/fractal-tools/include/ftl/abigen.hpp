...
namespace ftl {
    struct abigen_exception : public std::exception {
        virtual const char *what() const throw() {
            return "fractal-abigen fatal error";
        }
    };
    ...
    public:
        ...
        abigen() : generation_utils([&]() { throw abigen_exception(); }) {
        }
        ...
        void add_action(const clang::CXXRecordDecl *decl) {
            abi_action ret;
            auto action_name = decl->getFtlActionAttr()->getName();

            if (action_name.empty()) {
                try {
                    validate_name(decl->getName().str(), error_handler);
                } catch (...) {
                    std::cout << "Error, name <" << decl->getName().str() << "> is an invalid Fractal name.\n";
                    throw;
                }
                ...
                try {
                    ...
                    std::cout << "Error, name <" << action_name.str() << "> is an invalid Fractal name.\n";
                    throw;
                }
        ...
        void add_action(const clang::CXXMethodDecl *decl) {
            ...
            if (action_name.empty()) {
                try {
                    ...
                    std::cout << "Error, name <" << decl->getNameAsString() << "> is an invalid Fractal name.\n";
                }
        ...
        void add_table(const clang::CXXRecordDecl *decl) {
            abi_table t;
            t.name = decl->getNameAsString();
            for (auto field : decl->fields()) {
                if (field->getName() == "key") {
                    t.key_type = get_type(field->getType());
                }
                else if (field->getName() == "value") {
                    t.value_type = get_type(field->getType());
                }
            }
            ...
            if (!is_builtin_type(translate_type(type))) {
                if (is_aliasing(type))
                    add_typedef(type);
                                                    ...
                                                    {"vector", "set", "deque", "list", "optional", "binary_extension",
                                                     "ignore"})) {
                    add_type(get_template_argument(type).getAsType());
        ...
        std::string generate_json_comment() {
            std::stringstream ss;
            ss << "This file was generated automatically.";
            ss << " DO NOT EDIT ";
            return ss.str();
        }
        ...
        ojson table_to_json(const abi_table &t) {
            ojson o;
            o["name"] = t.name;
            o["key_type"] = t.key_type;
            o["value_type"] = t.value_type;
            return o;
        }

        bool is_empty() {
            return _abi.structs.empty() && _abi.typedefs.empty() && _abi.actions.empty() && _abi.tables.empty();
        }
        ...
        ojson to_json() {
            ...
            auto validate_struct = [&](abi_struct as) {
                ...
                for (auto t : _abi.tables) {
                    if (as.name == _translate_type(t.key_type))
                        return true;
                    if (as.name == _translate_type(t.value_type))
                        return true;
                }
                    ...
                    if (validate_struct(as))
                        for (auto f : as.fields)
                            if (remove_suffix(f.type) == td.new_type_name)
                                return true;
                for (auto t : _abi.tables)
                    if (t.value_type == td.new_type_name)
                        return true;
                for (auto a : _abi.actions)
