...
Error WasmObjectFile::parseFtlABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   ftl_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("ftl abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".ftl_abi") {
     if (Error Err = parseFtlABISection(Ctx))
        return Err;
