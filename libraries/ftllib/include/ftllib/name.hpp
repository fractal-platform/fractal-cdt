...
namespace ftl {
    ...
    public:
        ...
        constexpr explicit name(std::string_view str)
                : value(0) {
            if (str.size() > 13) {
                ftl::check(false, "string is too long to be a valid name");
            }
            if (str.empty()) {
                return;
            }
            ...
            if (str.size() == 13) {
                ...
                if (v > 0x0Full) {
                    ftl::check(false, "thirteenth character in name cannot be a letter that comes after j");
                }
                value |= v;
            }
        }
        ...
        static constexpr uint8_t char_to_value(char c) {
            if (c == '.')
                ...
                ftl::check(false, "character is not in allowed character set for names");
...
inline constexpr ftl::name operator ""_n() {
    constexpr auto x = ftl::name{std::string_view{ftl::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
    return x;
}
